...
- **实时行情** — WebSocket 推送自选币种价格，K 线随周期自动刷新。
  ...
  两家各有一条 WebSocket，按币种自动分流，实时价格与秒级盘口都可用
...
- **资讯 AI 汇总** — Cointelegraph / Decrypt / CoinDesk / 币安公告，按币种过滤，
  再由模型压成一段简报：整体鹰鸽倾向与分数、鹰鸽两侧的力量对比（按影响量级加权），
  以及**逐条资讯的鹰/鸽标注与归类理由**——每个结论都能顺着标签点回原文核对。
  同样内置成本控制：结果默认复用 20 分钟，并可一键重新汇总
